...
function getHistorySpoolMeta(spoolID, spoolsById) {
    ...
    const weight = spool.remaining_weight != null ? `${Math.round(spool.remaining_weight)}g remaining` : 'Weight unknown';
...
function renderHistoryPrinterOptions(printers) {
    ...
    const printerEntries = Object.entries(historyPrinters).sort(([, left], [, right]) => {
...
function updateHistoryDropdownSelection(dropdown, spoolId, spoolMeta) {
    ...
    dropdown.querySelectorAll('.dropdown-option').forEach(option => {
        option.classList.toggle('selected', (option.dataset.value || '') === (hiddenInput ? hiddenInput.value : ''));
    });
}
...
function createHistoryDropdownOption(spoolId, spoolMeta) {
    ...
    swatch.style.backgroundColor = spoolMeta.colorHex ? `#${spoolMeta.colorHex}` : '#ccc';
...
function createHistorySpoolDropdown(entry, spoolsById, row) {
    ...
    if (entry.spool_id != null && !spoolsById.has(entry.spool_id)) {
        ...
        optionsContainer.appendChild(createHistoryDropdownOption(entry.spool_id, missingMeta));
    }

    Array.from(spoolsById.values())
        .sort((left, right) => left.id - right.id)
        .forEach(spool => {
            optionsContainer.appendChild(createHistoryDropdownOption(spool.id, getHistorySpoolMeta(spool.id, spoolsById)));
        });
        ...
        optionsContainer.querySelectorAll('.dropdown-option').forEach(option => {
            ...
            if (searchTerm !== '') {
                if (/^\d+$/.test(searchTerm)) {
                    ...
                    isMatch = Boolean(idMatch && (idMatch[1] === searchTerm || idMatch[2] === searchTerm));
                } else {
    ...
    optionsContainer.querySelectorAll('.dropdown-option').forEach(option => {
        option.addEventListener('click', function(event) {
            ...
            updateHistoryDropdownSelection(dropdown, optionValue === '' ? null : optionValue, optionMeta);
            closeHistoryDropdown(dropdown);
...
function updateHistoryRowState(row) {
    ...
    if (!select || !button || !action) {
        return;
    }

    const hasChanged = select.value !== row.dataset.currentSpoolId;
...
function renderPrintHistory(history, spoolsById) {
    ...
    history.forEach(entry => {
        ...
        row.dataset.currentSpoolId = entry.spool_id == null ? '' : String(entry.spool_id);
        ...
        usedCell.textContent = `${Math.round(entry.filament_used)}g`;
        ...
        saveButton.addEventListener('click', function() {
            ...
            savePrintHistorySpool(entry.id, selectedValue, saveButton);
        });
        ...
        toolheadCell.textContent = entry.toolhead_name || `Toolhead ${entry.toolhead_id}`;
...
async function importPrintHistory() {
    ...
    try {
        ...
        setHistoryFeedback(`Failed to read import payload: ${error.message}`, 'error');
        return;
    }
    if (!payload) {
    ...
    return historyFetchJson('/api/print-history/import', {
        ...
        .then(response => {
            setHistoryFeedback(formatHistoryImportSummary(response.summary), 'success');
            if (fileInput) {
                fileInput.value = '';
            }
        ...
        .catch(error => {
            setHistoryFeedback(`Failed to import print history: ${error.message}`, 'error');
        })
        .finally(() => {
            importButton.disabled = false;
...
function loadPrintHistory() {
    ...
    if (tbody) {
        tbody.innerHTML = '<tr><td colspan="6" class="history-empty">Loading print history...</td></tr>';
    }
...
function savePrintHistorySpool(historyId, spoolId, button) {
    button.disabled = true;
    const originalLabel = button.textContent;
    button.textContent = 'Saving...';
    setHistoryFeedback('', '');

    return historyFetchJson(`/api/print-history/${historyId}/spool`, {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({spool_id: spoolId === '' ? null : parseInt(spoolId, 10)}),
    })
        .then(() => {
        ...
        .catch(error => {
            setHistoryFeedback(`Failed to update print history: ${error.message}`, 'error');
        })
        .finally(() => {
